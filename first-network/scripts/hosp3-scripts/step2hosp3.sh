...
set -x
peer channel fetch 0 $CHANNEL_NAME.block -o orderer.healthblock.com:7050 --ordererTLSHostnameOverride orderer.healthblock.com -c $CHANNEL_NAME --tls --cafile $ORDERER_CA >&log.txt
res=$?
{ set +x; } 2>/dev/null
cat log.txt
