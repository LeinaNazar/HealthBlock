...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o orderer.healthblock.com:7050 --ordererTLSHostnameOverride orderer.healthblock.com -c $CHANNEL --tls --cafile $ORDERER_CA
  { set +x; } 2>/dev/null
...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"hosp3MSP":.[1]}}}}}' config.json ./organizations/peerOrganizations/hosp3.healthblock.com/hosp3.json > modified_config.json
{ set +x; } 2>/dev/null
...
setGlobals 2
set -x
peer channel update -f hosp3_update_in_envelope.pb -c ${CHANNEL_NAME} -o orderer.healthblock.com:7050 --ordererTLSHostnameOverride orderer.healthblock.com --tls --cafile ${ORDERER_CA}
{ set +x; } 2>/dev/null
